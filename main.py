...
async def train_model(train_request: TrainRequest):
    """Treina um modelo LSTM com os dados fornecidos."""
    try:
...
def main():
    ...
    print(
        f"✅ Dados carregados: {len(prices)} dias de {dates[0].strftime('%Y-%m-%d')} a {dates[-1].strftime('%Y-%m-%d')}"
    )
    ...
    x_train, y_train, x_test, y_test, dates_train, dates_test = (
        lstm.separate_train_test_data(x, y, sequence_dates, train_size=0.8)
    )
    print("📋 Divisão dos dados:")
    print(
        f"  Treino: {len(x_train)} sequências ({dates_train[0].strftime('%Y-%m-%d')} a {dates_train[-1].strftime('%Y-%m-%d')})"
    )
    print(
        f"  Teste: {len(x_test)} sequências ({dates_test[0].strftime('%Y-%m-%d')} a {dates_test[-1].strftime('%Y-%m-%d')})"
    )
    ...
    print("-" * 60)
    print(
        f"{'Treino':<12} {train_metrics['MAE']:<8.3f} {train_metrics['RMSE']:<8.3f} {train_metrics['MAPE']:<10.2f} {train_metrics['R²']:<8.3f}"
    )
    print(
        f"{'Teste':<12} {test_metrics['MAE']:<8.3f} {test_metrics['RMSE']:<8.3f} {test_metrics['MAPE']:<10.2f} {test_metrics['R²']:<8.3f}"
    )
    print("=" * 60)
    ...
    plot.plot_predictions_vs_actual(
        dates_train,
        y_train_actual,
    ...
    plot.plot_predictions_vs_actual(
        dates_test, y_test_actual, y_test_pred_rescaled, f"Conjunto de Teste - {TICKER}"
    )
